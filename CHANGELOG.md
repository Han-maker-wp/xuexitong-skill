...
可选答案源模块(借鉴 Samueli924/chaoxing 的答案源架构,独立实现):

- 三层答案源优先级:用户答案文件(`my-answers.json`)→ 用户自配 HTTP 接口(兼容 tikuAdapter 格式)→ AI 推理
- 新增 `scripts/answer_source.js`:题面清洗(去题号/分值后缀)、本地缓存、判断题同义词归一(对/正确/√/true…)、题型-答案一致性校验(单选混入判断字面/多段答案即弃用)、接口限速
- 审核单标注每题答案来源与 `命中 N/M` 覆盖率统计
- **红线不变**:skill 不内置、不爬取、不分发任何题库;接口由用户自备,结果只进审核单
