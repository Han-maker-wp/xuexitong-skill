...
- **答案源优先**(可选,默认关闭;详见 references/answer-sources.md):用户配置了答案文件(`my-answers.json`)或自建答案接口时,先跑 `node scripts/answer_source.js --questions questions.json`——命中项直接作为候选答案(来源标"用户答案/接口命中",置信度高),未命中项才走 AI 推理。审核单顶部标注 `答案源命中 N/M`。
- **"看想分离"读题法**(LangHY 实测:单模型看图答题 50 题错 3 题,两步走显著更准):公式/图片题先用视觉**只转录文字**(prompt 明确"不要分析、不要给答案,只输出文字",公式转 LaTeX),再把纯文字题目交给推理作答。
- 先看课程资料(见 references/download.md)能否提供依据,能引用就引用。
- 每题产出:**答案 + 一句话理由 + 置信度(高/中/低)**;多选宁少勿滥;计算题写关键公式与代入过程;简答分点、150~400 字。
- **低置信度题在审核单里置顶**,提醒用户优先复核(LangHY 实测 AI 首次正确率约 65~75%,这是常态,靠人工复核兜底)。
- **填空题格式敏感**:系统可能严格比对("50" 与 "50 m/s" 视为不同),按题面口径与单位作答。
- 整套答案先写进答案审核单(第 7 节),再回填。

### 考试/章节测验页的模板差异(LangHY 2026-06 实测)

- 题目容器可能是 `div.singleQuesId[data=questionId]`(而非 questionLi),题型标签 `.newZy_TItle`(extract_questions.js 已兼容两种);
- 选项 `span.num_option`:textContent=显示字母,**data=提交值且重做后会随机化**,永远按显示文本匹配;
- frame 需**滚动触发懒加载**(循环滚到底 + 重扫,最多 20 次);JS 未加载完的标志:`typeof btnBlueSubmit !== 'function'`——此时提交必然失败,先等加载;
- 已完成章节测验的 frame URL 会变成 `selectWorkQuestionYiPiYue`(与进行中的 `doHomeWorkNew` 是两套状态),都有题目 DOM。

### 提交与复盘(仅用户明确授权后)

1. 用户审核答案单后说"提交"→ 点提交按钮(其 onclick 多为 `btnBlueSubmit()`,弹出确认层;确认按钮 `#popok` 是 jQuery 绑定、无 onclick 属性——**必须真实点击**(locator click/force click),`dispatchEvent` 在 iframe 里静默失败);
2. 提交后若系统公布标准答案且有"重做"按钮:读取标准答案,生成对照复盘(错题分析);用户**再次明确授权**后才可重做重交(LangHY"重做拿满分"机制的人在回路版);
3. "待批阅"状态的章节(含填空/简答)**没有重做按钮**,无法复盘,如实告知。
